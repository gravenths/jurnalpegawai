...
class IndexController extends BaseController{
		...
		if(!empty($user)){
			...
			if(password_verify($password_text,$password_hash)){
				...
				if($rememberme == true){
					...
					$db->where("kodeguru", $user['kodeguru']);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("kodeguru","password","nama","tempatlahir","tanggallahir","alamat","email","nohp","photo","nip","user_role_id"); //registration fields
		...
		$page_title = $this->view->page_title = "Tambah Pegawai";
